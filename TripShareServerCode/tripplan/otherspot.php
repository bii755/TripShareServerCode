...
$sql = "select * from $countrycode where pl_sub_category_nm = '$kind' order by pl_clip_cnt desc limit 15";
}else if ($kind == "전체") {
//종류만 전체이고 도시를 선택했을 경우
$response[] = "selected city and no kind";
$sql = "select * from $countrycode where city = '$city' order by pl_clip_cnt desc limit 15";
}else {
//도시, 종류 둘 다 전체가 아니 각각 다른 것을 선택했을 경우
$response[] = "selected city and kind";
$sql = "select * from $countrycode where city = '$city' and pl_sub_category_nm = '$kind' order by pl_clip_cnt desc limit 15";
}
